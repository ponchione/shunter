...
export const SHUNTER_CLIENT_MESSAGE_CALL_PROCEDURE = 11;
...
function closeReasonIndicatesAuthFailure(reason) {
...
function authErrorFromCloseEvent(event, message) {
...
export function createShunterClient(options) {
    ...
    const pendingProcedureCalls = new Map();
    ...
    const cleanupSocketListeners = (ws, handlers) => {
        try {
            ...
            // Listener cleanup is best-effort for host-provided WebSocket adapters.
        }
        try {
    ...
    const allocateMessageKeyRequestId = (pending, abandoned, exhaustionMessage, exhaustionCode) => {
    ...
    const allocateDeclaredQueryRequestId = () => allocateMessageKeyRequestId(pendingDeclaredQueries, abandonedDeclaredQueries, "No declared query message IDs are available.", "declared_query_message_ids_exhausted");
    const allocateProcedureRequestId = () => allocateMessageKeyRequestId(pendingProcedureCalls, abandonedProcedureCalls, "No procedure message IDs are available.", "procedure_message_ids_exhausted");
    ...
    const correlatedEntryForResponse = (requestId, queryId, label, noun, byRequest, byQuery) => {
    ...
    const pendingSubscriptionForResponse = (requestId, queryId, label) => correlatedEntryForResponse(requestId, queryId, label, "pending subscription", pendingSubscriptionsByRequest, pendingSubscriptionsByQuery);
    const abandonedSubscriptionForResponse = (requestId, queryId, label) => correlatedEntryForResponse(requestId, queryId, label, "abandoned subscription", abandonedSubscriptionsByRequest, abandonedSubscriptionsByQuery);
    const pendingUnsubscribeForResponse = (requestId, queryId, label) => correlatedEntryForResponse(requestId, queryId, label, "pending unsubscribe", pendingUnsubscribesByRequest, pendingUnsubscribesByQuery);
            ...
            const prepareAndOpen = async () => {
                ...
                try {
                    ...
                    failConnecting(isShunterError(error)
                        ? error
                        : new ShunterValidationError("Connection configuration is invalid.", { cause: error }));
                    return;
                }
                let token;
                try {
                    token = await resolveToken(options.token);
                }
                catch (error) {
                ...
                try {
                    ...
                    failConnecting(new ShunterValidationError("WebSocket URL is invalid.", {
                ...
                if (options.signal?.aborted) {
                    failConnecting(new ShunterClosedClientError("Connection aborted before opening."));
                    return;
                }
                let ws;
                try {
                    ...
                    failConnecting(toShunterError(error, "transport", "Create WebSocket failed"));
                    return;
                }
                socket = ws;
                ...
                const cleanupOpeningAbort = () => {
                    try {
                        ...
                        // Abort-listener cleanup cannot block transport teardown.
                    }
                ...
                const handlers = {
                    ...
                    close: (event) => {
                        ...
                        if (state.status === "connecting") {
                            const error = authErrorFromCloseEvent(event, "WebSocket authentication failed before opening.") ?? new ShunterTransportError("WebSocket closed before opening.", {
                                code: String(event.code),
                                details: { reason: event.reason, wasClean: event.wasClean },
                            });
                            ...
                            const error = authErrorFromCloseEvent(event, "Shunter client authentication failed.") ?? new ShunterClosedClientError("Shunter client connection closed.", {
                                code: String(event.code),
                                details: { reason: event.reason, wasClean: event.wasClean },
                            });
                            if (!scheduleReconnect(error)) {
                ...
                try {
                    ...
                    cleanupOpeningAbort();
                    cleanupSocketListeners(ws, handlers);
                    failConnecting(new ShunterTransportError("Configure WebSocket failed.", { cause: error }));
                }
            };
            void prepareAndOpen().catch((error) => {
                if (connectGeneration !== generation || state.status !== "connecting") {
                    return;
                }
                failConnecting(isShunterError(error)
                    ? error
                    : new ShunterTransportError("Open WebSocket failed.", { cause: error }));
            });
        });
        return connectPromise;
    };
    return {
        ...
        async callProcedure(name, args, options = {}) {
            if (disposed) {
...
export function decodeProcedureResponseFrame(data) {
    const frame = frameBytes(data);
    ...
    offset = resultOffset;
    const [totalHostExecutionDuration, durationOffset] = readInt64LE(frame, offset, "ProcedureResponse total_host_execution_duration");
    offset = durationOffset;
    if (offset !== frame.length) {
...
export function encodeProcedureCallRequest(name, args, options = {}) {
    const requestId = options.requestId;
    if (requestId !== undefined) {
