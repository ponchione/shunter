...
- Other open scout candidates (separate, not-yet-bundled):
  ...
  - `:sender` parameter matching should be exact-case, e.g. `SELECT * FROM s WHERE id = :SENDER` should return raw OneOff `Unsupported expression: :SENDER` and SubscribeSingle ``Unsupported expression: :SENDER, executing: `SELECT * FROM s WHERE id = :SENDER` ``. Current Shunter accepts/registers the query because `parseLiteral` matches `sender` with `strings.EqualFold`; reference `parse_expr` only accepts the exact placeholder string `":sender"` (`sql-parser/src/parser/mod.rs:223`) and routes other placeholders through `SqlUnsupported::Expr`.
  ...
  - `SELECT DISTINCT` must stay rejected as an unsupported modifier rather than being reinterpreted as a projection column, e.g. with table `t` containing columns `DISTINCT` and `u32`, `SELECT DISTINCT u32 FROM t` should return raw OneOff `Unsupported: SELECT DISTINCT u32 FROM t` and SubscribeSingle ``Unsupported SELECT: SELECT DISTINCT u32 FROM t, executing: `SELECT DISTINCT u32 FROM t` ``. Current Shunter parses `DISTINCT` as a column name with output alias `u32`, so OneOff can succeed when that column exists; SubscribeSingle currently falls through to the column-projection return-type guard. Reference SQL and subscription parsers both require `distinct: None` in `parse_select` (`sql-parser/src/parser/sql.rs:365`, `sql-parser/src/parser/sub.rs:122`).
  - `SELECT ALL` must stay rejected as an unsupported modifier rather than being reinterpreted as a projection column, e.g. with table `t` containing column `ALL`, `SELECT ALL u32 FROM t` should return raw OneOff `Unsupported: SELECT ALL u32 FROM t` and SubscribeSingle ``Unsupported SELECT: SELECT ALL u32 FROM t, executing: `SELECT ALL u32 FROM t` ``. Current Shunter parses `ALL` as a column name with output alias `u32`, so OneOff succeeds when that column exists and SubscribeSingle reports the column-projection return-type guard. Reference SQL and subscription parsers both require `distinct: None` in `parse_select` (`sql-parser/src/parser/sql.rs:365`, `sql-parser/src/parser/sub.rs:122`), and `SELECT ALL ...` is represented as a non-`None` set quantifier.
  ...
  - unqualified names in join scope should use the reference parser text `Names must be qualified when using joins`, e.g. `SELECT id FROM t JOIN s ON t.id = s.id`, `SELECT t.* FROM t JOIN s ON t.id = s.id WHERE id = 7`, and `SELECT t.* FROM t JOIN s ON id = s.id` should return that raw OneOff text and the SubscribeSingle `WithSql` counterpart. Current Shunter emits three different parser-local strings (`join projections must be qualified`, `join WHERE columns must be qualified`, and `expected qualified column reference`) under a `parse: unsupported SQL:` prefix. Reference `SqlSelect::find_unqualified_vars` routes all three through `SqlUnsupported::UnqualifiedNames`.
  ...
  - bare join wildcard rejection should not mask earlier `JOIN ON` type errors, e.g. `SELECT * FROM t JOIN s ON t.missing = s.id` should return raw OneOff `` `missing` is not in scope `` and SubscribeSingle `` `missing` is not in scope, executing: `SELECT * FROM t JOIN s ON t.missing = s.id` ``. Current Shunter checks the `SELECT *` join wildcard guard first and emits `SELECT * is not supported for joins` on both surfaces. Reference `type_from` types the `ON` expression before projection typing (`expr/src/check.rs:100-104`), so the `type_proj` join-wildcard guard is not reached for this shape.
  ...
  - boolean simplification must not hide `JOIN ON` type errors when a later `WHERE FALSE` is present, e.g. `SELECT t.* FROM t JOIN s ON t.missing = s.id WHERE FALSE` should return raw OneOff `` `missing` is not in scope `` and SubscribeSingle `` `missing` is not in scope, executing: `SELECT t.* FROM t JOIN s ON t.missing = s.id WHERE FALSE` ``. Current Shunter combines the parsed `ON` predicate with the `WHERE` predicate, normalizes the whole tree to `FalsePredicate`, and succeeds/registers. Reference `type_from` types the `ON` expression before `type_select` sees the `WHERE` clause (`expr/src/check.rs:100-104`).
  - SubscribeSingle's projection-return guard should not mask earlier FROM/WHERE type errors on projection queries, e.g. `SELECT u32 FROM missing_table` should emit ``no such table: `missing_table`. If the table exists, it may be marked private., executing: `SELECT u32 FROM missing_table` `` and `SELECT u32 FROM t WHERE missing = 1` should emit `` `missing` is not in scope, executing: `SELECT u32 FROM t WHERE missing = 1` ``. Current Shunter returns `Column projections are not supported in subscriptions; Subscriptions must return a table type` before relation and filter type checking. Reference `SubChecker::type_set` runs `type_from` / `type_select` before `type_proj` and the table-type return guard.
  ...
  - SubscribeSingle `SELECT missing FROM t` projection-column resolution should surface reference `Unresolved::Var` BEFORE the `Unsupported::ReturnType` guard fires. OneOff side closed by Slice D. SubscribeSingle still rejects earlier with `Column projections are not supported in subscriptions; Subscriptions must return a table type` because `compileSQLQueryString`'s `allowProjection=false` branch fires before `compileProjectionColumns` resolves the column. Closing the SubscribeSingle side requires reordering the column-resolution pass before the column-projection guard, which would require updating the existing `TestHandleSubscribeSingle_Parity{Aggregate,ColumnList}ReturnTypeRejectText` pins to use a column whose name DOES exist on the table.
