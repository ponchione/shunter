...
- **TD-141** [FOLLOWUP][resolved] `protocol/client_messages.go`, `protocol/server_messages.go`, `protocol/handle_oneoff.go` — Phase 2 Slice 1c, wire-shape parity, landed 2026-04-20. `OneOffQueryMsg` / `OneOffQueryResult` now use `MessageID []byte`, client/server codecs encode it as a length-prefixed byte string, the one-off handler preserves the opaque bytes through success/error replies, and the positive pin `TestPhase2Slice1COneOffQueryMessageIDBytes` locks both request/response shapes. Verification: `rtk go test ./protocol -run TestPhase2Slice1COneOffQueryMessageIDBytes -v`; `rtk go test ./protocol -run 'TestOneOffQuery|TestPhase2Slice1'`; `rtk go test ./protocol`; `rtk go test ./...`.

- **TD-143** [FOLLOWUP][resolved] `commitlog/recovery.go`, `store/recovery.go`, `commitlog/recovery_test.go` — `P0-RECOVERY-002` closed on 2026-04-20. Snapshot + replay recovery now keeps the recovered `TxID` horizon, restores persisted `nextID`, and advances per-table auto-increment sequences only when replayed inserts match the current recovered sequence value. This fixes the restart mismatch where replaying an explicit non-zero autoincrement row (for example `id=42`) incorrectly bumped the recovered sequence to `43`, causing the next zero-valued insert after restart to diverge from live non-restarted behavior. Locked by `TestOpenAndRecoverDetailedSnapshotReplayIgnoresExplicitAutoincrementRowsWhenRestoringSequence` plus the existing `TestOpenAndRecoverSnapshotAndLogRecovery`. Verification: `rtk go test ./store ./commitlog -run 'Recovery|Replay|Snapshot|Sequence|TxID|nextID' -v`; `rtk go test ./store ./commitlog`; `rtk go test ./...`.

- **TD-142** [FOLLOWUP] `query/sql/parser.go`, `protocol/handle_subscribe.go`, `protocol/handle_oneoff.go`, `subscription/predicate.go` — Narrow parity widening now has twelve landed sub-slices as of 2026-04-20: same-table qualified WHERE columns; case-insensitive unquoted identifier resolution through the schema seam; single-table alias / qualified-star support; ordered single-column comparisons `<`, `<=`, `>`, `>=` lowering into `subscription.ColRange`; non-equality comparisons `<>` / `!=` lowering into `subscription.ColNe`; narrow same-table `OR` predicates such as `SELECT * FROM metrics WHERE score = 9 OR score = 11` lowering into a real `subscription.Or` tree across parser, subscribe, and one-off query handling; a first narrow join-backed SQL slice for left-projected equi-joins such as `SELECT o.* FROM Orders o JOIN Inventory product ON o.product_id = product.id WHERE product.quantity < 10`; a second narrow join-backed SQL slice for right-side equi-join projection such as `SELECT product.* FROM Orders o JOIN Inventory product ON o.product_id = product.id`, including the filtered variant `SELECT product.* FROM Orders o JOIN Inventory product ON o.product_id = product.id WHERE o.id = 1`; a third narrow join-backed slice for cross-join projection such as `SELECT o.* FROM Orders o JOIN Inventory product` (no `ON`, no extra `WHERE`); and a fourth narrow join-backed slice for aliased self cross-join projection such as `SELECT a.* FROM t AS a JOIN t AS b` (no `ON`, no extra `WHERE`) — lowered into the existing `subscription.CrossJoinProjected` with `Projected == Other`, which the register-set / one-off / delta-eval paths already dedupe into a single-table scan gated on the table being non-empty. Alias-scope parity is now also tightened to match the reference: once a relation is aliased, the base table name is no longer a valid qualifier for projection or qualified `WHERE` references (for example `SELECT item.* FROM users AS item WHERE users.id = 1` is now rejected). Unaliased self cross-joins are now also rejected in line with the reference expectation that self-joins require aliases (`SELECT t.* FROM t JOIN t` now fails). Positive pins for the recent aliased self cross-join slice: `query/sql/parser_test.go::TestParseAliasedSelfCrossJoinProjection`, `protocol/handle_subscribe_test.go::TestHandleSubscribeSingle_AliasedSelfCrossJoin`, `protocol/handle_oneoff_test.go::TestHandleOneOffQuery_AliasedSelfCrossJoinProjection`, and `protocol/handle_oneoff_test.go::TestHandleOneOffQuery_AliasedSelfCrossJoinEmptyTable`. Negative pins for the self-join rejection tighten: `query/sql/parser_test.go::TestParseRejectsUnaliasedSelfCrossJoin` and `protocol/handle_subscribe_test.go::TestHandleSubscribeSingle_UnaliasedSelfCrossJoinRejected`. Remaining grounded reference-backed SQL gaps still center on richer alias-aware runtime semantics: aliased self equi-join (`SELECT a.* FROM t AS a JOIN t AS b ON a.u32 = b.u32`) remains rejected at the parser because the ON-column cross-relation check resolves both qualifiers to the same concrete table (`JOIN ON must compare columns from different relations`), which is the first shape that actually forces the alias-vs-table-id identity split in `subscription.Join` / runtime; multi-join reference shapes (`SELECT t.* FROM t JOIN s JOIN s AS r WHERE ...`) are still unsupported and also require alias-aware runtime identity. Do not reopen the already-landed non-join slices while taking that next step. Verification: `rtk go test ./query/sql ./protocol ./subscription -v` and `rtk go test ./...` (10 packages, 1075 tests passing as of 2026-04-20).

  **Slice 12 landed 2026-04-20.** Alias-aware WHERE on self-join is now live. `subscription.ColEq` / `ColNe` / `ColRange` gained `Alias uint8`, `subscription/hash.go::encodePredicate` writes the alias byte into each leaf's canonical form, and `subscription/validate.go::validateSelfJoinFilterAliases` checks that self-join filter leaves tag one of the enclosing `Join`'s aliases. `subscription.MatchRow` is a thin wrapper over the new `MatchRowSide(pred, table, sideAlias, row)` which adds the alias-vs-side check (`if p.Alias != sideAlias { return true }`) alongside the existing cross-table pass-through; `subscription/delta_join.go::tryJoinFilter` threads `join.LeftAlias` / `join.RightAlias` to `MatchRowSide` so each side of a joined pair sees only its own tagged leaves. `subscription/placement.go::PlaceSubscription` / `RemoveSubscription` short-circuit self-join (`Join.Left == Join.Right`) to Tier 3 because alias-tagged filter leaves cannot safely drive Tier 1 / Tier 2 pruning (a row's insertion can play the unconstrained side). `query/sql.Filter` gained `Alias string` (the user-typed qualifier); `protocol/handle_subscribe.go::compileSQLQueryString` builds an `aliasTag func(string) uint8` for the enclosing join — identity tag for the Left alias, `1` for the Right alias in self-join (distinct-table joins leave everything at zero) — threaded through `compileSQLPredicateForRelations` / `normalizeSQLFilterForRelations` / `normalizePredicate`. `protocol/handle_oneoff.go::evaluateOneOffJoin` calls `subscription.MatchRowSide` with per-side aliases. The previous `self-join with WHERE not yet supported` rejection at the compile boundary is removed. Shapes `SELECT a.* FROM t AS a JOIN t AS b ON a.u32 = b.u32 WHERE a.id = 1` and the symmetric `WHERE b.id = 1` now work through parser, subscribe admission, one-off execution, and post-commit IVM delta evaluation. Pinned: `subscription/eval_test.go::TestEvalSelfEquiJoinWithAliasedWhere` (IVM delta math — inserting `r2(id=2, u32=5)` into `t={r1(id=1, u32=5)}` with filter `a.id=1` yields exactly `{(r1,r2)}` rather than the 3-row filterless bag), `protocol/handle_subscribe_test.go::TestHandleSubscribeSingle_AliasedSelfEquiJoinWithWhere` (subscribe admission now constructs `Join.Filter = ColEq{Alias: LeftAlias}` instead of rejecting), `protocol/handle_oneoff_test.go::TestHandleOneOffQuery_AliasedSelfEquiJoinWithWhereAside` (filter constrains only a-side: 1 row returned), and `protocol/handle_oneoff_test.go::TestHandleOneOffQuery_AliasedSelfEquiJoinWithWhereBside` (filter constrains only b-side: all 3 a-side rows emitted when b.id=1 is a common partner). Slice 11's `TestEvalSelfEquiJoinSubscription` still passes unchanged. Remaining Phase 2 anchor: Slice 13 multi-join (three-way, with self-alias) such as `SELECT t.* FROM t JOIN s JOIN s AS r WHERE t.u32 = s.u32 AND s.u32 = r.u32`, which requires a multi-relation runtime representation because `subscription.Join` is a binary predicate. Verification: `rtk go test ./query/sql ./protocol ./subscription -v`; `rtk go test ./...` (10 packages, 1084 tests passing).

  **Slice 11 landed 2026-04-20.** Runtime alias identity is now live. `subscription.Join` gained `LeftAlias uint8` / `RightAlias uint8`; `Tables()` deduplicates to a single entry when `Left == Right`; `subscription/hash.go::encodePredicate(Join)` includes both aliases in the canonical form; `subscription/validate.go::validateJoin` rejects `Left == Right` with equal aliases. `protocol/handle_subscribe.go::compileSQLQueryString` now accepts aliased self equi-join — it produces `Join{Left: t, Right: t, LeftAlias: 0, RightAlias: 1, ...}` when `leftID == rightID`. The `self-join runtime not yet supported` Slice 10.5 boundary rejection is removed; a narrower `self-join with WHERE not yet supported` rejection remains because per-alias WHERE filters collapse to `Table=t` under the existing `compileSQLPredicateForRelations` map and `MatchRow` would apply the filter to both sides of a joined pair — a real parity gap deferred to a later slice (alias-aware filter compile). Initial-query and post-commit delta evaluation work correctly without modification because the 4-fragment IVM math at `subscription/delta_join.go:22-58` handles `Left == Right` naturally: `dv.InsertedRows(Left)` and `dv.InsertedRows(Right)` return the same physical delta (`dr == ds` in reference notation), and `ReconcileJoinDelta` bag-semantics (`subscription/delta_dedup.go:9-66`) cancel the double-counted `D3 = dr(+) ⋈ ds(+)` fragment against the doubled inserts, yielding the expected `V' − V` bag multiplicity. Pinned: `query/sql/parser_test.go::TestParseAliasedSelfEquiJoinProjection` (parser), `protocol/handle_subscribe_test.go::TestHandleSubscribeSingle_AliasedSelfEquiJoin` (protocol → manager predicate shape), `protocol/handle_oneoff_test.go::TestHandleOneOffQuery_AliasedSelfEquiJoin` (one-off execution, 3 rows where every u32 has at least one matching partner), and `subscription/eval_test.go::TestEvalSelfEquiJoinSubscription` (IVM delta math — insert `r2(5)` into `t={r1(5)}` yields the bag `{(r1,r2), (r2,r1), (r2,r2)}` after reconciliation). Boundary regressions: `TestHandleSubscribeSingle_AliasedSelfEquiJoinWithWhereRejected`, `TestHandleOneOffQuery_AliasedSelfEquiJoinWithWhereRejected`. Verification: `rtk go test ./query/sql ./protocol ./subscription -v`; `rtk go test ./...` (10 packages, 1082 tests passing).

  **Slice 10.5 landed 2026-04-20.** Parser-only widening for aliased self equi-join now in place. `query/sql.JoinClause` gained `LeftAlias`/`RightAlias` string fields and `ColumnRef` gained an `Alias` field so downstream can distinguish relation instances even when `LeftTable == RightTable`. Parser's ON-column cross-relation check is now alias-based (`query/sql/parser.go::parseJoinClause`): it rejects only when both ON qualifiers resolve to the same alias, which is why `SELECT a.* FROM t AS a JOIN t AS b ON a.u32 = a.u32` is rejected (`TestParseRejectsSameAliasBothSidesOfEquiJoin`) while `SELECT a.* FROM t AS a JOIN t AS b ON a.u32 = b.u32` now parses (`TestParseAliasedSelfEquiJoinProjection`). The unaliased self-join rejection (`SELECT t.* FROM t JOIN t`) still holds because the default alias for an unaliased relation is the base table name and both defaults collide. Compile path in `protocol/handle_subscribe.go::compileSQLQueryString` explicitly rejects aliased self equi-join with `self-join runtime not yet supported` when `leftID == rightID` on an ON-join, pending Slice 11's runtime alias identity split. Pinned at the subscribe and one-off boundaries: `protocol/handle_subscribe_test.go::TestHandleSubscribeSingle_AliasedSelfEquiJoinRejectedAtBoundary` and `protocol/handle_oneoff_test.go::TestHandleOneOffQuery_AliasedSelfEquiJoinRejectedAtBoundary`. Aliased self cross-join (no ON) continues to work end-to-end through `subscription.CrossJoinProjected`. Verification: `rtk go test ./query/sql ./protocol -run 'TestParseAliasedSelfEquiJoinProjection|TestParseRejectsSameAliasBothSidesOfEquiJoin|TestHandleSubscribeSingle_AliasedSelfEquiJoinRejectedAtBoundary|TestHandleOneOffQuery_AliasedSelfEquiJoinRejectedAtBoundary' -v`; `rtk go test ./...` (10 packages, 1079 tests passing).

  **Aliased self equi-join: two-slice plan (decided 2026-04-20).** Reading `reference/SpacetimeDB/crates/subscription/src/lib.rs:165-253` confirmed the parity-correct direction is shape A (relation-instance identity separate from `TableID`). Reference `Fragments::compile_from_plan` keys every `TableScan`/`IxScan` on `Label` (alias), and `mut_plan` applies the IVM delta by Label match, so `t AS a JOIN t AS b` produces two scans with distinct Labels that share one physical table's delta. Single-slice landing was rejected after the investigation surfaced three non-trivial runtime concerns that should not ride on the same commit: (1) `subscription/delta_join.go:25-28` keys delta scans on `TableID`, so self-join 4-fragment IVM naturally double-counts when `Left == Right`; reference tolerates this because `for_each_insert` emits the bag without dedup, but Shunter's `ReconcileJoinDelta` bag semantics (`subscription/delta_dedup.go:9-66`) need explicit verification that the per-row-key multiplicity math still collapses correctly for self-join inputs. (2) `subscription/eval.go:337-342` returns `Inserts: ins` where `ins` carries LHS++RHS concatenated rows (`tryJoinFilter` at `subscription/delta_join.go:167-180`), not rows projected to the SELECT side — a pre-existing projection gap that affects every join subscription today, not just self-joins, and is worth flagging before the self-join slice amplifies it. (3) `query/sql/parser.go:441` rejects ON pairs where both qualifiers resolve to the same concrete table, which is the first knob to turn on. **Slice 10.5 (next)**: parser-only widening — extend `JoinClause` with alias-identity metadata (`LeftAlias` / `RightAlias`), relax the "ON must compare different relations" check to be alias-aware (reject only when both qualifiers are the *same* alias), accept `SELECT a.* FROM t AS a JOIN t AS b ON a.u32 = b.u32` through the parser, and let `compileSQLQueryString` in `protocol/handle_subscribe.go` reject with a narrow `self-join runtime not yet supported` error at the subscribe/one-off boundary. Positive parser pin, negative subscribe/one-off pins. Preserves: unaliased self-join still rejected at parser, aliased self cross-join still works end-to-end. **Slice 11 (after)**: runtime alias identity — add relation-instance tags to `subscription.Join`, thread through `delta_join.go` / `register_set.go` / `eval.go` / `placement.go` / `handle_oneoff.go::evaluateOneOffJoin`, verify `ReconcileJoinDelta` bag-semantics produce the reference-expected multiplicity for self-join fragments, and (optionally, if cheap and in-scope) close the LHS++RHS-vs-projected row gap at the same time. Multi-join (three-way, with self-alias) is a later slice after Slice 11 lands and stabilizes.
