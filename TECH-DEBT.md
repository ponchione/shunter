...
Scout findings (2026-04-27):
- Quoted table identifiers still case-fold after parsing. `query/sql` tokenization records whether an identifier was quoted, but `Statement` carries only the text; `schema.SchemaRegistry.TableByName` then exact-matches first and falls back to `strings.EqualFold`. User-visible risk: `SELECT * FROM "PLAYERS"` can resolve table `players` instead of rejecting a quoted-case mismatch. Cover OneOff, SubscribeSingle, and SubscribeMulti when fixing.
- Quoted column identifiers still case-fold after parsing. `TableSchema.Column` always accepts `strings.EqualFold`, and quoted status is not carried into `Filter`, `ColumnRef`, or `ProjectionColumn`. User-visible risk: `SELECT * FROM t WHERE "U32" = 7`, `SELECT "U32" FROM t`, or `JOIN ... ON t."U32" = s.u32` can bind to `u32` even though the user wrote a quoted case-specific name.
- Alias and qualifier lookup remains case-insensitive outside the closed case-distinct collision/routing pins. Parser qualifier maps and single-table compile checks use `strings.EqualFold`. User-visible risk: `SELECT R.* FROM t AS r`, `SELECT * FROM t AS r WHERE R.id = 1`, or self-join predicates can route through a case-mismatched qualifier instead of rejecting it as out of scope.
- Projection duplicate detection is byte-exact while column lookup is case-folded. User-visible risk: OneOff can accept `SELECT u32, U32 FROM t` and project the same physical column twice, or produce a policy-inconsistent result, depending on the final identifier rule. This belongs with the identifier-policy slice, not the general join-projection slice.
- Join/cross-join WHERE remains separate from identifier handling. OneOff currently admits some cross-join WHERE column-equality forms that SubscribeSingle rejects before registration; inner-join WHERE field-vs-field predicates parse but are not generally lowered/evaluated. Treat this as the next separate OI-002 batch only after the identifier slice or if a worker is explicitly assigned that scope.
