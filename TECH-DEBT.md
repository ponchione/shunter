...
- Other open scout candidates (separate, not-yet-bundled):
  ...
  - SubscribeSingle LIMIT rejection should precede SELECT set-quantifier rejection when both are present, e.g. `SELECT DISTINCT * FROM t LIMIT 5` (and the same `ALL` family) should return ``Unsupported: SELECT DISTINCT * FROM t LIMIT 5, executing: `SELECT DISTINCT * FROM t LIMIT 5` ``. Current Shunter parses the SELECT first and emits ``Unsupported SELECT: SELECT DISTINCT * FROM t LIMIT 5, executing: ...``. Reference `SubParser::parse_query` rejects any subscription `Query` with `limit: Some(...)` through `SubscriptionUnsupported::feature(query)` before `parse_select` can route `distinct: Some(...)` to `SubscriptionUnsupported::Select` (`sql-parser/src/parser/sub.rs:94-107,120-149`).
