...
- dispatch-handler ctx audit: `protocol/dispatch.go:168` spawns handler goroutines with `ctx` hardcoded to `context.Background()` at `protocol/upgrade.go:201`. Handlers forward that ctx into `inbox.CallReducer` / `inbox.RegisterSubscriptionSet` / `inbox.UnregisterSubscriptionSet`. Different hang class from the disconnect path (request/response rather than teardown); narrow sub-slice if a specific unbounded-block seam surfaces
- `forwardReducerResponse` ctx audit: `executor/protocol_inbox_adapter.go:128` spawns `go a.forwardReducerResponse(ctx, ...)` with the Background ctx from the dispatch loop; if the executor never feeds the internal `respCh` the goroutine leaks. Analog to the 2026-04-20 `watchReducerResponse` fix on the other seam
