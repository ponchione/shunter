...
- Slice B (algebraic-name compound rendering + compound error classes) — open scout candidates:
  - malformed `LitString -> KindTimestamp` should emit reference `InvalidLiteral` text with the timestamp Product type, e.g. ``The literal expression `not-a-timestamp` cannot be parsed as type `(__timestamp_micros_since_unix_epoch__: I64)` ``. Current Shunter emits generic `unsupported SQL: malformed timestamp literal ...`. Scope is bounded to coerce-layer parity plus OneOff raw and SubscribeSingle `WithSql` pins; no protocol wrapper change is expected because `InvalidLiteralError` already passes through `normalizeSQLFilterForRelations`.
  - numeric literals targeting `KindTimestamp` should emit `InvalidLiteral` with the timestamp Product type, e.g. `WHERE ts = 42` and `WHERE ts = 1.3` should reject as type `(__timestamp_micros_since_unix_epoch__: I64)`. Current Shunter emits generic integer/float-to-timestamp coercion errors with a `coerce column` prefix.
  - `UnexpectedType` for bool literals targeting `KindTimestamp` should also use the timestamp Product type, e.g. `WHERE ts = TRUE` should emit `Bool != (__timestamp_micros_since_unix_epoch__: I64)`; current Shunter renders `Bool != Timestamp`.
  - scalar literals targeting `KindArrayString` should use reference array rendering and error classes: `WHERE arr = 'x'` should emit ``The literal expression `x` cannot be parsed as type `Array<String>` ``, while `WHERE arr = TRUE` should emit `Unexpected type: (expected) Bool != Array<String> (inferred)`. Current Shunter emits generic scalar-to-array coercion for strings and renders bool mismatch as `ArrayString`.
- Slice C (compile-stage validation text) — open scout candidates:
  - duplicate projection aliases should emit reference `DuplicateName` text, e.g. ``Duplicate name `dup` `` for `SELECT u32 AS dup, i32 AS dup FROM t`; current Shunter accepts and returns duplicate output names.
  - duplicate implicit projection names should emit reference `DuplicateName` text, e.g. ``Duplicate name `u32` `` for `SELECT u32, u32 FROM t`; current Shunter accepts and returns duplicate output names.
  - duplicate join aliases should emit reference `DuplicateName` text, e.g. ``Duplicate name `dup` `` for `FROM t AS dup JOIN s AS dup`; current Shunter rejects earlier with `parse: unsupported SQL: joined relations must use distinct qualifiers`.
  - unaliased self-joins should hit the same `DuplicateName` surface, e.g. ``Duplicate name `t` `` for `FROM t JOIN t`; current Shunter rejects earlier with `parse: unsupported SQL: self join requires aliases`.
  - join column type mismatches should emit reference `UnexpectedType` text, e.g. `JOIN s ON t.u32 = s.name` should reject as `Unexpected type: (expected) String != U32 (inferred)`. Current Shunter rejects later through subscription predicate validation with `subscription: invalid predicate: join column kinds differ`.
  - array/product join comparability text should match the fixed reference literal `Product values are not comparable`; current Shunter emits contextual text such as `join ON t.arr = s.arr: array/product values are not comparable`.
