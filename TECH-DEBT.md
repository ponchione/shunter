...
- broader A2/runtime-model gaps remain; after the bounded cross-join `WHERE` + `COUNT(*)` aggregate combination closure, the next bounded residual should be chosen from fresh live evidence rather than carried forward from stale pre-closure handoff notes
