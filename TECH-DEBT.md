...
- **TD-142** [FOLLOWUP] `query/sql/parser.go`, `schema/registry.go`, `schema/types.go` — Narrow parity widening now has five landed sub-slices as of 2026-04-20: (1) same-table qualified WHERE columns are accepted (`SELECT * FROM users WHERE users.id = 1`), pinned by `query/sql/parser_test.go::TestParseWhereQualifiedColumnsSameTable` and `protocol/handle_subscribe_test.go::TestHandleSubscribeSingle_QualifiedColumnsSameTable`; (2) unquoted SQL table/column identifiers now resolve case-insensitively through the schema seam, so mixed-case forms like `SELECT * FROM USERS WHERE ID = 1 AND users.DISPLAY_NAME = 'alice'` compile against lowercase registered schema names, pinned by `schema/types_test.go::TestTableSchemaColumnLookupCaseInsensitive`, `schema/build_test.go::TestRegistryTableByNameCaseInsensitive`, and `protocol/handle_subscribe_test.go::TestHandleSubscribeSingle_MixedCaseTableAndColumns`; (3) single-table alias/qualified-star forms now parse and normalize correctly (`SELECT item.* FROM Inventory AS item WHERE item.id = 7` and `SELECT item.* FROM Inventory item`), pinned by `query/sql/parser_test.go::TestParseSelectQualifiedStarWithAlias`, `query/sql/parser_test.go::TestParseSelectQualifiedStarWithAsAliasAndQualifiedWhereColumns`, `protocol/handle_subscribe_test.go::TestHandleSubscribeSingle_QualifiedStarAlias`, and `protocol/handle_oneoff_test.go::TestHandleOneOffQuery_QualifiedStarAlias`; (4) ordered single-column comparison operators now normalize through subscribe and one-off query handling for `<`, `<=`, `>`, and `>=`, mapped onto `subscription.ColRange`, pinned by `query/sql/parser_test.go::TestParseWhereComparisonOperators`, `protocol/handle_subscribe_test.go::TestHandleSubscribeSingle_GreaterThanComparison`, and `protocol/handle_oneoff_test.go::TestHandleOneOffQuery_LessThanOrEqualComparison`; and (5) non-equality comparison operators `<>` and `!=` now normalize through subscribe and one-off query handling, mapped onto `subscription.ColNe`, pinned by `query/sql/parser_test.go::TestParseWhereNotEqualOperators`, `protocol/handle_subscribe_test.go::TestHandleSubscribeSingle_NotEqualComparison`, and `protocol/handle_oneoff_test.go::TestHandleOneOffQuery_NotEqualComparison`. The grammar is still intentionally small: `SELECT ( * | qualifier.* ) FROM T [ [AS] alias ] [WHERE colref op lit (AND colref op lit)*]` where `op` is one of `=`, `!=`, `<>`, `<`, `<=`, `>`, or `>=`, qualifiers may only match the `FROM` table or its alias, and broader reference-backed shapes remain open. Remaining grounded parity gaps against `reference/SpacetimeDB/` are `OR` predicates and join-backed subscription/query forms; keep future work paired across parser + protocol/runtime coverage rather than widening the parser alone.
