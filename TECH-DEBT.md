...
Remaining sub-hazards:
- `committed_state.go` shared-state escape route: `CommittedState.Table(id) *Table` raw-pointer exposure — the returned `*Table` is accessed outside the `CommittedState` RLock envelope by every caller; under single-writer discipline this is safe, but the contract is unpinned
