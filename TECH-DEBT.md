...
- Other open scout candidates (separate, not-yet-bundled):
  ...
  - JOIN projection validation should not mask qualified WHERE resolution, e.g. `SELECT * FROM t JOIN s ON t.id = s.id WHERE t.missing = 1` should return raw OneOff `` `missing` is not in scope `` and SubscribeSingle `` `missing` is not in scope, executing: `SELECT * FROM t JOIN s ON t.id = s.id WHERE t.missing = 1` `` instead of the bare-wildcard join rejection. The same ordering applies to explicit projection resolution: `SELECT t.proj_missing FROM t JOIN s ON t.id = s.id WHERE s.where_missing = 1` should report `` `where_missing` is not in scope `` before walking `t.proj_missing`. Current Shunter validates JOIN projections in `compileSQLQueryString` before compiling the qualified WHERE predicate, so it emits `SELECT * is not supported for joins` or `` `proj_missing` is not in scope `` first. Reference `SqlChecker::type_set` and `SubChecker::type_set` run `type_select(Self::type_from(...), expr, vars)` before `type_proj` whenever a filter is present (`expr/src/statement.rs:431-437`, `expr/src/check.rs:145-151`).
