...
- accepted single-table same-table absorption-equivalent `AND` / `OR` SQL now canonicalizes bounded absorption-law variants at that same identity seam, so `a OR (a AND b)` and `a AND (a OR b)` share one canonical query hash and one shared query state with `a` while one-off row meaning remains unchanged
...
- bare/grouped reference-backed `FALSE` predicates now compile and execute coherently on the already-supported SQL surface: `query/sql/parser.go` accepts `FALSE`, `protocol/handle_subscribe.go` normalizes `FALSE AND/OR` identities and lowers admitted false-only queries to `subscription.NoRows`, and `subscription/hash.go` / `validate.go` / `delta_single.go` / `manager.go` now share that no-result runtime meaning across hashing, validation, bootstrap, and evaluation
- accepted distinct-table join SQL whose filter differs only by same-table commutative child order now also shares one canonical query hash and one shared query state: `subscription/hash.go` canonicalizes `Join.Filter` for distinct-table joins without rewriting the join structure itself, while one-off row semantics stay unchanged and self-join alias-sensitive filter canonicalization remains intentionally out of scope
- row-level security / per-client filtering remains absent
- broader query/subscription parity is still open beyond the landed narrow shapes, especially predicate normalization / validation drift and other bounded A2 gaps that remain after the closed join-index-validation + committed-ordering + projected-join-delta-ordering + sender-parameter-hash-identity + neutral-TRUE-normalization + commutative-child-order + associative-grouping + duplicate-leaf-idempotence + same-table-absorption + overlength-SQL-admission + false-predicate-follow-through + distinct-table-join-filter-child-order seams

Execution note:
- OI-002 remains the next active handoff issue, but the fan-out delivery batch, the join/cross-join multiplicity batch, the one-off-vs-subscribe join-index validation seam, the committed join bootstrap/final-delta projected-order seam, the projected-join delta-order seam, the sender-parameter hash-identity seam, the neutral-`TRUE` normalization seam, the accepted same-table commutative child-order seam, the accepted same-table associative-grouping seam, the accepted same-table duplicate-leaf idempotence seam, the accepted same-table absorption-law seam, the overlength-SQL admission seam, the false-predicate follow-through seam, and the distinct-table join-filter child-order canonicalization seam are now closed. The next bounded A2 batch should start from another remaining runtime/model gap after a fresh scout rather than reopening those closed slices or closed A1 protocol work.
