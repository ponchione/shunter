...
func TestOpenAndRecoverDurabilityBoundaryFaultMatrix(t *testing.T) {
	...
	cases := []faultCase{
		...
		{
			name: "snapshot-covered-valid-prefix-with-truncated-final-record-recovers-prefix",
			setup: func(t *testing.T, root string, reg schema.SchemaRegistry) {
				writeFaultSnapshot(t, root, reg, 2, map[uint64]string{1: "alice", 2: "bob"})
				path := writeReplaySegment(t, root, 3,
					replayRecord{txID: 3, inserts: []types.ProductValue{{types.NewUint64(3), types.NewString("carol")}}},
			...
			assert: func(t *testing.T, recovered *store.CommittedState, maxTxID types.TxID, plan RecoveryResumePlan, report RecoveryReport, err error) {
				...
				if report.ReplayedTxRange != (RecoveryTxIDRange{Start: 3, End: 4}) {
					t.Fatalf("replayed range = %+v, want 3..4", report.ReplayedTxRange)
				}
		...
		{
			name: "snapshot-covered-valid-prefix-with-truncated-final-crc-recovers-prefix",
			setup: func(t *testing.T, root string, reg schema.SchemaRegistry) {
				writeFaultSnapshot(t, root, reg, 2, map[uint64]string{1: "alice", 2: "bob"})
				path := writeReplaySegment(t, root, 3,
					replayRecord{txID: 3, inserts: []types.ProductValue{{types.NewUint64(3), types.NewString("carol")}}},
				...
				truncateScanTestFileToOffset(t, path, int64(scanTestRecordCRCOffset(t, path, 2, RecordCRCSize-1)))
			},
			assert: func(t *testing.T, recovered *store.CommittedState, maxTxID types.TxID, plan RecoveryResumePlan, report RecoveryReport, err error) {
				if err != nil {
					t.Fatal(err)
		...
		{
			name: "snapshot-covered-zero-length-rollover-recovers-snapshot",
			setup: func(t *testing.T, root string, reg schema.SchemaRegistry) {
				writeFaultSnapshot(t, root, reg, 2, map[uint64]string{1: "alice", 2: "bob"})
				createZeroLengthSegment(t, root, 3)
			},
			assert: func(t *testing.T, recovered *store.CommittedState, maxTxID types.TxID, plan RecoveryResumePlan, report RecoveryReport, err error) {
				if err != nil {
					t.Fatal(err)
		...
		{
			name: "snapshot-log-gap-to-zero-length-rollover-fails-loudly",
			setup: func(t *testing.T, root string, reg schema.SchemaRegistry) {
				writeFaultSnapshot(t, root, reg, 2, map[uint64]string{1: "alice", 2: "bob"})
				createZeroLengthSegment(t, root, 4)
			},
			assert: func(t *testing.T, recovered *store.CommittedState, maxTxID types.TxID, plan RecoveryResumePlan, report RecoveryReport, err error) {
				if err == nil {
					t.Fatal("expected snapshot/log boundary gap error")
		...
		{
			name: "full-log-prefix-with-zero-length-rollover-recovers-prefix",
			setup: func(t *testing.T, root string, reg schema.SchemaRegistry) {
				writeReplaySegment(t, root, 1,
					replayRecord{txID: 1, inserts: []types.ProductValue{{types.NewUint64(1), types.NewString("alice")}}},
					replayRecord{txID: 2, inserts: []types.ProductValue{{types.NewUint64(2), types.NewString("bob")}}},
		...
		{
			name: "zero-length-first-segment-fails-loudly",
			setup: func(t *testing.T, root string, reg schema.SchemaRegistry) {
				createZeroLengthSegment(t, root, 1)
			},
			assert: func(t *testing.T, recovered *store.CommittedState, maxTxID types.TxID, plan RecoveryResumePlan, report RecoveryReport, err error) {
				if err == nil {
					t.Fatal("expected zero-length first segment error")
				}
		...
		{
			name: "partial-zero-filled-active-tail-recovers-valid-prefix",
			setup: func(t *testing.T, root string, reg schema.SchemaRegistry) {
				path := writeReplaySegment(t, root, 1,
					replayRecord{txID: 1, inserts: []types.ProductValue{{types.NewUint64(1), types.NewString("alice")}}},
					replayRecord{txID: 2, inserts: []types.ProductValue{{types.NewUint64(2), types.NewString("bob")}}},
				)
				appendZeroTail(t, path, RecordHeaderSize-1)
			},
			assert: func(t *testing.T, recovered *store.CommittedState, maxTxID types.TxID, plan RecoveryResumePlan, report RecoveryReport, err error) {
				if err != nil {
					t.Fatal(err)
				...
				assertReplayPlayerRows(t, recovered, map[uint64]string{1: "alice", 2: "bob"})
				if len(report.DamagedTailSegments) != 0 {
					t.Fatalf("damaged tail report = %+v, want none for partial zero-filled tail", report.DamagedTailSegments)
				}
				if plan.AppendMode != AppendInPlace || plan.SegmentStartTx != 1 || plan.NextTxID != 3 {
					t.Fatalf("resume plan = %+v, want append-in-place on segment 1 at tx 3", plan)
				}
...
func TestOpenAndRecoverSnapshotPastDurableHorizonMatrix(t *testing.T) {
	...
	cases := []horizonCase{
		...
		{
			name: "newest-snapshot-past-horizon-falls-back-to-older-snapshot-and-damaged-tail-prefix",
			setup: func(t *testing.T, root string, reg schema.SchemaRegistry) {
				writeFaultSnapshot(t, root, reg, 5, map[uint64]string{1: "alice"})
				writeFaultSnapshot(t, root, reg, 10, map[uint64]string{99: "too-new"})
				path := writeReplaySegment(t, root, 6,
					replayRecord{txID: 6, inserts: []types.ProductValue{{types.NewUint64(2), types.NewString("bob")}}},
					replayRecord{txID: 7, inserts: []types.ProductValue{{types.NewUint64(3), types.NewString("partial-carol")}}},
				)
				truncateScanTestFileToOffset(t, path, int64(scanTestRecordOffset(t, path, 1)+RecordHeaderSize-1))
			},
			assert: func(t *testing.T, recovered *store.CommittedState, maxTxID types.TxID, plan RecoveryResumePlan, report RecoveryReport, err error) {
				if err != nil {
					t.Fatal(err)
				...
				if report.ReplayedTxRange != (RecoveryTxIDRange{Start: 6, End: 6}) {
					t.Fatalf("replayed range = %+v, want 6..6", report.ReplayedTxRange)
				}
				if len(report.DamagedTailSegments) != 1 || report.DamagedTailSegments[0].StartTx != 6 || report.DamagedTailSegments[0].LastTx != 6 {
					t.Fatalf("damaged tail report = %+v, want damaged replay tail prefix 6..6", report.DamagedTailSegments)
				}
...
func TestOpenAndRecoverSnapshotTailZeroHeaderNonzeroPadding(t *testing.T) {
	root := t.TempDir()
	_, reg := testSchema()
	writeFaultSnapshot(t, root, reg, 2, map[uint64]string{1: "alice", 2: "bob"})
	tailPath := writeReplaySegment(t, root, 3,
	...
	appendZeroHeaderNonZeroTail(t, tailPath)
	...
	if len(report.DamagedTailSegments) != 1 || report.DamagedTailSegments[0].StartTx != 3 || report.DamagedTailSegments[0].LastTx != 4 {
		t.Fatalf("damaged tail report = %+v, want one tail segment 3..4", report.DamagedTailSegments)
	}
	if plan.AppendMode != AppendByFreshNextSegment || plan.SegmentStartTx != 5 || plan.NextTxID != 5 {
		t.Fatalf("resume plan = %+v, want fresh segment at tx 5", plan)
	}
...
func TestOpenAndRecoverFallsBackWhenOffsetIndexPointsAtZeroTailSentinel(t *testing.T) {
	root := t.TempDir()
	_, reg := testSchema()
	writeFaultSnapshot(t, root, reg, 2, map[uint64]string{1: "alice", 2: "bob"})
	segmentPath := writeReplaySegment(t, root, 1,
	...
	zeroTailOffset := uint64(info.Size())
	appendZeroTail(t, segmentPath, RecordOverhead)

	idxPath := filepath.Join(root, OffsetIndexFileName(1))
	idx := populateSparseIndex(t, idxPath, 4, []OffsetIndexEntry{
		{TxID: 3, ByteOffset: zeroTailOffset},
	})
	if err := idx.Close(); err != nil {
		t.Fatal(err)
	}
	...
	recovered, maxTxID, plan, report, err := OpenAndRecoverWithReport(root, reg)
	if err != nil {
		t.Fatalf("OpenAndRecoverWithReport with zero-tail offset index %s -> %d for segment %s: %v", idxPath, zeroTailOffset, segmentPath, err)
	}
	if maxTxID != 3 {
		t.Fatalf("maxTxID = %d, want 3 (report=%+v)", maxTxID, report)
	}
	assertReplayPlayerRows(t, recovered, map[uint64]string{1: "alice", 2: "bob", 3: "carol"})
	if !report.HasSelectedSnapshot || report.SelectedSnapshotTxID != 2 {
		t.Fatalf("selected snapshot report = (%v, %d), want (true, 2)", report.HasSelectedSnapshot, report.SelectedSnapshotTxID)
	...
	if len(report.DamagedTailSegments) != 0 {
		t.Fatalf("damaged tail report = %+v, want none for safe zero tail", report.DamagedTailSegments)
	}
	if plan.AppendMode != AppendInPlace || plan.SegmentStartTx != 1 || plan.NextTxID != 4 {
		t.Fatalf("resume plan = %+v, want append-in-place on segment 1 at tx 4", plan)
	}
...
func TestOpenAndRecoverFallsBackWhenOffsetIndexPathIsUnopenable(t *testing.T) {
	root := t.TempDir()
	_, reg := testSchema()
	writeFaultSnapshot(t, root, reg, 2, map[uint64]string{1: "alice", 2: "bob"})
	writeReplaySegment(t, root, 1,
...
func TestCreateSnapshotTempOpenSyncAndCloseFailuresLeaveNoSelectableSnapshotAndCompleteLogRecovers(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			tmpPath := filepath.Join(snapshotDir, snapshotTempFileName)
			faultErr := errors.New(tc.name + " failed")
			tc.configure(t, writer, tmpPath, faultErr)
			...
			if HasLockFile(snapshotDir) {
				t.Fatal("snapshot lock should be removed after temp fault")
			}
			for _, name := range []string{snapshotTempFileName, snapshotFileName} {
				if _, err := os.Stat(filepath.Join(snapshotDir, name)); !os.IsNotExist(err) {
					t.Fatalf("%s should not exist after %s failure, stat err=%v", name, tc.name, err)
				}
			}
...
func TestOpenAndRecoverAfterCompactionSyncFailureWithSidecarsUsesSnapshotAndTail(t *testing.T) {
	root := t.TempDir()
	_, reg := testSchema()
	writeFaultSnapshot(t, root, reg, 3, map[uint64]string{1: "alice", 2: "bob", 3: "carol"})
	covered := writeReplaySegment(t, root, 1,
	...
	tailIdx := filepath.Join(root, OffsetIndexFileName(4))
	for _, path := range []string{coveredIdx, tailIdx} {
	...
	if _, err := os.Stat(coveredIdx); !os.IsNotExist(err) {
		t.Fatalf("covered sidecar should be removed before sync failure is reported, stat err=%v", err)
	}
	if _, err := os.Stat(tail); err != nil {
		t.Fatalf("tail segment should remain after compaction sync failure: %v", err)
	}
	if _, err := os.Stat(tailIdx); err != nil {
		t.Fatalf("tail sidecar should remain after compaction sync failure: %v", err)
	}
