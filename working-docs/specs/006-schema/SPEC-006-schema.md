...
type ColumnSchema struct {
    ...
    Nullable      bool      // column accepts null values carrying this Type
...
- Nullable columns are allowed. Store validation rejects null values for non-nullable columns and accepts null values for nullable columns only when the null carries the declared column kind.
- `autoincrement` columns must not be nullable (`ErrNullableAutoIncrement`).
...
| `ErrNullableAutoIncrement` | A column combines `Nullable` and `AutoIncrement` |
