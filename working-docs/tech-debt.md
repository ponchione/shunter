...
## TypeScript Package Version Policy

`typescript/client` is a private local package named `@shunter/client`. Its
package version is currently independent-looking, while Shunter's runtime/tool
version lives in the repo root `VERSION`.

Before the next client-facing TypeScript release work, decide whether
`typescript/client/package.json` should track Shunter releases or stay on an
independent private-package version. If it tracks Shunter, update the smoke
test expectations and install examples in the same change.

## Related Active Planning

Typed dynamic parameters for declared queries and views are the next active
implementation plan and are tracked separately in
`working-docs/v1.1-declared-read-parameters.md`. Keep the detailed design there
