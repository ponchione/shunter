...
// JoinPath5Edge identifies a five-hop directional join traversal used when a
// changed relation reaches a filtered relation through four intermediate
// relations.
...
// JoinPath5EdgeIndex is the value-filter pruning index for five-hop path
// edges.
type JoinPath5EdgeIndex struct {
	edges   map[JoinPath5Edge]map[valueKey]map[QueryHash]struct{}
	byTable map[TableID]map[JoinPath5Edge]int
}

// NewJoinPath5EdgeIndex constructs an empty JoinPath5EdgeIndex.
func NewJoinPath5EdgeIndex() *JoinPath5EdgeIndex {
	return &JoinPath5EdgeIndex{
		edges:   make(map[JoinPath5Edge]map[valueKey]map[QueryHash]struct{}),
		byTable: make(map[TableID]map[JoinPath5Edge]int),
	}
}
...
func (ji *JoinPath5EdgeIndex) addEdgeRef(edge JoinPath5Edge) {
	perEdge, ok := ji.byTable[edge.LHSTable]
	if !ok {
		perEdge = make(map[JoinPath5Edge]int)
...
func (ji *JoinPath5EdgeIndex) removeEdgeRef(edge JoinPath5Edge) {
...
// JoinRangePath5EdgeIndex is the range-filter companion to
// JoinPath5EdgeIndex.
type JoinRangePath5EdgeIndex struct {
	edges   map[JoinPath5Edge]map[rangeKey]*rangeBucket
	byTable map[TableID]map[JoinPath5Edge]int
}

// NewJoinRangePath5EdgeIndex constructs an empty JoinRangePath5EdgeIndex.
func NewJoinRangePath5EdgeIndex() *JoinRangePath5EdgeIndex {
	return &JoinRangePath5EdgeIndex{
		edges:   make(map[JoinPath5Edge]map[rangeKey]*rangeBucket),
		byTable: make(map[TableID]map[JoinPath5Edge]int),
	}
}
