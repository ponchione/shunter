...
// JoinPath7Edge identifies a seven-hop directional join traversal used when a
// changed relation reaches a filtered relation through six intermediate
// relations.
...
// JoinPath7EdgeIndex is the value-filter pruning index for seven-hop path
// edges.
type JoinPath7EdgeIndex struct {
	edges   map[JoinPath7Edge]map[valueKey]map[QueryHash]struct{}
	byTable map[TableID]map[JoinPath7Edge]int
}

// NewJoinPath7EdgeIndex constructs an empty JoinPath7EdgeIndex.
func NewJoinPath7EdgeIndex() *JoinPath7EdgeIndex {
	return &JoinPath7EdgeIndex{
		edges:   make(map[JoinPath7Edge]map[valueKey]map[QueryHash]struct{}),
		byTable: make(map[TableID]map[JoinPath7Edge]int),
	}
}
...
func (ji *JoinPath7EdgeIndex) addEdgeRef(edge JoinPath7Edge) {
	perEdge, ok := ji.byTable[edge.LHSTable]
	if !ok {
		perEdge = make(map[JoinPath7Edge]int)
...
func (ji *JoinPath7EdgeIndex) removeEdgeRef(edge JoinPath7Edge) {
...
// JoinRangePath7EdgeIndex is the range-filter companion to
// JoinPath7EdgeIndex.
type JoinRangePath7EdgeIndex struct {
	edges   map[JoinPath7Edge]map[rangeKey]*rangeBucket
	byTable map[TableID]map[JoinPath7Edge]int
}

// NewJoinRangePath7EdgeIndex constructs an empty JoinRangePath7EdgeIndex.
func NewJoinRangePath7EdgeIndex() *JoinRangePath7EdgeIndex {
	return &JoinRangePath7EdgeIndex{
		edges:   make(map[JoinPath7Edge]map[rangeKey]*rangeBucket),
		byTable: make(map[TableID]map[JoinPath7Edge]int),
	}
}
