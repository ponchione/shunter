	...
	for _, t := range pred.Tables() {
		...
		if join != nil {
			...
			if placements, ok := splitJoinOrPlacementsFor(pred, join, t, resolver); ok {
				mutateSplitJoinOrPlacements(idx, placements, t, hash, add)
				continue
			}
...
func multiJoinFilterEdgesBetweenRelations(
	...
	for _, path := range multiJoinFilterEdgeNonKeyPaths(conditions, lhsRelation, rhsRelation) {
		baseEdge, ok := path.traversalEdge(0, resolver)
		if !ok {
			continue
		}
		for _, ce := range filters.eqs {
			...
			out.pathTraversalEdges = append(out.pathTraversalEdges, joinPathTraversalEdgePlacement{
				edge:  edge,
				value: ce.Value,
			})
		}
		for _, cr := range filters.ranges {
			...
			out.rangePathTraversalEdges = append(out.rangePathTraversalEdges, joinRangePathTraversalEdgePlacement{
				edge:  edge,
				lower: cr.Lower,
				upper: cr.Upper,
			})
...
type multiJoinFilterEdgePathHop struct {
	from MultiJoinColumnRef
	to   MultiJoinColumnRef
}

type multiJoinFilterEdgePath struct {
	hops []multiJoinFilterEdgePathHop
}

func (path multiJoinFilterEdgePath) nonKeyPreserving() bool {
	for i := 1; i < len(path.hops); i++ {
		if path.hops[i-1].to.Column != path.hops[i].from.Column {
			return true
		}
	}
	return false
...
func (path multiJoinFilterEdgePath) traversalEdge(filterCol ColID, resolver IndexResolver) (joinPathTraversalEdge, bool) {
	hops := len(path.hops)
	tables := make([]TableID, hops+1)
	fromCols := make([]ColID, hops)
	toCols := make([]ColID, hops)
	for i, hop := range path.hops {
		if _, ok := resolver.IndexIDForColumn(hop.to.Table, hop.to.Column); !ok {
			return joinPathTraversalEdge{}, false
		}
		if i == 0 {
			tables[0] = hop.from.Table
		}
		tables[i+1] = hop.to.Table
		fromCols[i] = hop.from.Column
		toCols[i] = hop.to.Column
	}
	return newJoinPathTraversalEdge(tables, fromCols, toCols, filterCol)
}

func multiJoinFilterEdgeNonKeyPaths(conditions []MultiJoinCondition, lhsRelation int, rhsRelation int) []multiJoinFilterEdgePath {
	if lhsRelation == rhsRelation {
		return nil
	}
	var paths []multiJoinFilterEdgePath
	visited := map[int]struct{}{lhsRelation: {}}
	var walk func(currentRelation int, hops []multiJoinFilterEdgePathHop)
	walk = func(currentRelation int, hops []multiJoinFilterEdgePathHop) {
		if len(hops) >= joinPathTraversalMaxHops {
			return
		}
		for _, condition := range conditions {
			current, next, ok := multiJoinConditionRefsFromRelation(condition, currentRelation)
			if !ok || current.Relation == next.Relation {
				continue
			}
			nextHops := appendPathHop(hops, multiJoinFilterEdgePathHop{from: current, to: next})
			if next.Relation == rhsRelation {
				if len(nextHops) >= 2 {
					path := multiJoinFilterEdgePath{hops: nextHops}
					if path.nonKeyPreserving() {
						paths = append(paths, path)
					}
				}
				continue
			}
			if _, ok := visited[next.Relation]; ok {
				continue
			}
			visited[next.Relation] = struct{}{}
			walk(next.Relation, nextHops)
			delete(visited, next.Relation)
		}
	}
	walk(lhsRelation, nil)
	return paths
}

func appendPathHop(hops []multiJoinFilterEdgePathHop, hop multiJoinFilterEdgePathHop) []multiJoinFilterEdgePathHop {
	next := make([]multiJoinFilterEdgePathHop, len(hops)+1)
	copy(next, hops)
	next[len(hops)] = hop
	return next
}
...
func mutateSplitJoinOrPlacements(idx *PruningIndexes, placements splitJoinOrPlacements, table TableID, hash QueryHash, add bool) {
	for _, ce := range placements.eqs {
		mutateValuePlacement(idx, table, ce.Column, ce.Value, hash, add)
	}
	for _, cr := range placements.ranges {
		mutateRangePlacement(idx, table, cr.Column, cr.Lower, cr.Upper, hash, add)
	}
	...
	for _, placement := range placements.pathTraversalEdges {
		mutateJoinPathTraversalEdgePlacement(idx, placement.edge, placement.value, hash, add)
	}
...
func mutateJoinPathTraversalEdgePlacement(idx *PruningIndexes, edge joinPathTraversalEdge, value Value, hash QueryHash, add bool) {
	if add {
...
func collectCandidatesForTableInto(
	...
	collectJoinPathTraversalCandidates(idx, table, rows, committed, resolver, func(h QueryHash) {
		set[h] = struct{}{}
	})
...
type joinPathTraversalEdgePlacement struct {
	edge  joinPathTraversalEdge
	value Value
}

type joinRangePathTraversalEdgePlacement struct {
	edge  joinPathTraversalEdge
	lower Bound
	upper Bound
}
