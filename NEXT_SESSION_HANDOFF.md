...
- `SELECT *` on JOIN rejection now emits the reference literal `"SELECT * is not supported for joins"` across SubscribeSingle / SubscribeMulti (with `DBError::WithSql` suffix) and OneOff (raw, no suffix) closed in `3b3b96d`. Reference anchor: `reference/SpacetimeDB/crates/expr/src/errors.rs:41` (`InvalidWildcard::Join`), emit site `reference/SpacetimeDB/crates/expr/src/lib.rs:56` via `type_proj` when `ast::Project::Star(None)` meets `input.nfields() > 1`. Refactor pattern: the parser-boundary rejection at `query/sql/parser.go:586-588` moved to the compile stage in `protocol/handle_subscribe.go::compileSQLQueryString` (post-table-resolution, pre-projection compile) so the text no longer carries the `"parse: unsupported SQL: "` prefixes. Pins: `protocol/handle_subscribe_test.go::TestHandleSubscribe{Single,Multi}_ParityJoinStarProjectionRejectText` and `protocol/handle_oneoff_test.go::TestHandleOneOffQuery_ParityJoinStarProjectionRejectText`; parser-accept invariant at `query/sql/parser_test.go::TestParseAcceptsJoinBareStarProjection`.

Do not reopen `P0-SUBSCRIPTION-001` through `P0-SUBSCRIPTION-033`, the reused-hash elision, the `SubscriptionError.table_id: None` closure, the compile-origin `WithSql` suffix, the Subscribe{Single,Multi} initial-eval error closures, the Unsubscribe{Single,Multi} final-eval error closures, or the `SELECT *` on JOIN reject-text closure without fresh failing regression evidence.

## Scout Prompts

Before starting implementation, answer one of these with live-code evidence:

- Is there another reference type-check rejection literal from `reference/SpacetimeDB/crates/expr/src/errors.rs` whose Shunter analogue emits divergent text on the same input shape? The `3b3b96d` closure followed this playbook: pick a fixed-literal (or identifier-/type-name-only interpolation) rejection template → confirm Shunter rejects the same input with divergent text (typically carrying a `"parse: unsupported SQL: "` prefix) → move the check from parser boundary to compile stage in `protocol/handle_subscribe.go::compileSQLQueryString` so the prefix chain drops. Remaining candidate templates, each bounded only where Shunter already emits under a detectable-divergent text for the same input: `Unresolved::Var` (`errors.rs:12` `"`{0}` is not in scope"`), `Unresolved::Table` (`:14` `"no such table: `{0}`. If the table exists, it may be marked private."`), `Unresolved::Field` (`:16` `"`{0}` does not have a field `{1}`"`), `Unsupported::ReturnType` (`:47` `"Column projections are not supported in subscriptions; Subscriptions must return a table type"`), `Unsupported::ProjectExpr` (`:49` `"Unsupported expression in projection"`), `InvalidOp` (`:72` `"Invalid binary operator `{op}` for type `{ty}`"`), `InvalidLiteral` (`:88` `"The literal expression `{literal}` cannot be parsed as type `{ty}`"`), `UnexpectedType` (`:104` `"Unexpected type: (expected) {expected} != {inferred} (inferred)"`), `DuplicateName` (`:120` `"Duplicate name `{0}`"`). Table- and field-not-found (`:14`/`:16`) are the most likely already-divergent because Shunter emits `"unknown table %q"` / `"unknown column %q on table %q"` today. Verify with a grep-before-implement scan.
...
- **Admission-error-text parity on the subscription-manager bookkeeping seam (ErrSubscriptionNotFound, ErrQueryIDAlreadyLive, Client not found, query-state-not-found).** Distinct from the type-check rejections in `expr/src/errors.rs` covered by the first scout prompt above — these fire from the `add_subscription`/`remove_subscription` bookkeeping inside the manager, not from SQL type-check. Every reference emit site formats Rust tuples via Debug `{:?}`: `module_subscription_manager.rs:850`, `:857`, `:863`, `:875`, `:908`, `:915`, `:921`, `:928`, `:989`, `:1055`. The only fixed-literal admission emit is `module_subscription_actor.rs:750` `"Internal error"` on the UnsubscribeSingle duplicate-subscription path, but the reference comment at `:747` explicitly states "This whole function should be removed soon". Not a bounded parity surface.
