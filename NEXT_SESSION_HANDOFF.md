...
Prior A1 slices (`SubscriptionError` duration field, `CallReducer` field order, `TransactionUpdate` field order, applied-envelope field order, `IdentityToken` rename + field order, `Unsubscribe.SendDropped` removal, `OneOffQueryResponse` rename + field-order close, Phase 2 Slice 4 rows-shape documented divergence, `SubscriptionError` TransactionUpdate-origin RequestID/QueryID None parity) are closed. OI-001 still has residual visible divergences. Pick one and repeat the scout → pick → close-or-pin pattern.
