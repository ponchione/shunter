...
Prior A1 slices (`SubscriptionError` duration field, `CallReducer` field order, `TransactionUpdate` field order, applied-envelope field order, `IdentityToken` rename + field order, `Unsubscribe.SendDropped` removal, `OneOffQueryResponse` rename + field-order close, Phase 2 Slice 4 rows-shape documented divergence) are closed. OI-001 still has residual visible divergences. Pick one and repeat the scout → pick → close-or-pin pattern.
...
- `SubscriptionError` emission populates `RequestID` as non-nil via `optionalUint32(subErr.RequestID)` on every fan-out path (`protocol/fanout_adapter.go:92`). Reference sets `request_id = None` when the error originated inside a `TransactionUpdate` rather than a Subscribe/Unsubscribe request. Behavioral gap at the emit site, not a wire-shape gap; the field is already `*uint32` on the wire.
- Client `ServerMessage::Subscribe` (legacy non-Single variant) is absent from Shunter; flagged in reference as "will be removed when we switch to `SubscribeSingle`". Already out-of-scope per OI-001 A1, but verify no client still negotiates for it before closing the audit.
