...
`InvalidLiteral` text parity has been extended to every bounded no-widening-trap target through `sql.mismatch`:
...
Every further `InvalidLiteral` candidate carries either a **widening trap** (reference accepts, Shunter rejects) or needs infrastructure work. Pick the next slice from **Scout Directions** below. Do not widen beyond a single reference anchor per slice.
...
- **Widening** slice: LitInt/LitFloat/LitBytes → `KindString`. Reference `parse(value, String)` wraps the source text as `AlgebraicValue::String` and succeeds; Shunter's `case types.KindString:` in `coerceValue` rejects any non-LitString via `mismatch`. This is a behavior change, not a text slice — closing it drops the `case types.KindString` non-LitString reject and routes through the same source-text rendering as `renderLiteralSourceText` (plus a canonical-hex decision for LitBytes). Coordinate with the `LitBytes → KindString` case which currently rejects but reference accepts.
- **Widening** slice: LitString containing a numeric token → integer/float column (`WHERE u32 = '123'`). Reference `parse_int`/`parse_float` invokes `BigDecimal::from_str(value)` which accepts digit-only strings; Shunter rejects any non-LitInt/LitBigInt/LitFloat on numeric columns via the existing `coerceSigned`/`coerceUnsigned`/LitFloat arms. Closing this slice is also a behavior change. Note that after it closes, LitString → integer on non-numeric strings still emits `InvalidLiteral` via the same `mismatch` branch added for the LitFloat→integer slice — so the same `InvalidLiteralError` + `renderLiteralSourceText` infrastructure applies on the reject-path.
