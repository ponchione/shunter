...
Tier-B hardening narrow sub-slice of `OI-004`: dispatch-handler ctx sub-hazard closed. Request-side analog to the earlier-same-day `forwardReducerResponse` response-side slice — the per-message handler goroutines spawned in `runDispatchLoop` no longer leak when the executor inbox wedges, because their ctx now cancels on `c.closed` in addition to the outer ctx.

- Decision doc: `docs/hardening-oi-004-dispatch-handler-context.md`
- Sharp edge: `protocol/dispatch.go:192` spawns one goroutine per inbound message. Each `run` closure captures the outer `ctx` received by `runDispatchLoop`, hardcoded to `context.Background()` at `protocol/upgrade.go:201`. Every handler except `handleOneOffQuery` forwards that ctx into `ExecutorInbox.CallReducer` / `ExecutorInbox.RegisterSubscriptionSet` / `ExecutorInbox.UnregisterSubscriptionSet`, which the `ProtocolInboxAdapter` translates to `executor.SubmitWithContext(ctx, cmd)`. In the default (non-reject) inbox mode, that seam does `select { case e.inbox <- cmd: return nil; case <-ctx.Done(): return ctx.Err() }`. With Background root, the ctx.Done() arm never fires; when the executor wedges (hung reducer, engine stall, scheduler-blocking commit) and the inbox fills to `InboxCapacity`, subsequent handler goroutines park on `e.inbox <- cmd` indefinitely. Each hung goroutine holds an `inflightSem` slot (`dispatch.go:186-190`) and a `*Conn` capture; after `IncomingQueueMessages` concurrent hangs the connection's read loop closes with `1008 "too many requests"` and the supervisor drives Disconnect — but the hung goroutines do not observe `c.closed` and stay parked forever, pinning the `*Conn` and the `inflightSem` past teardown. Same "Background-rooted caller" shape as the prior OI-004 sub-slices; different select seam (inside `SubmitWithContext`, before the command is even accepted).
- Fix: derive a dispatch-scoped `handlerCtx` in `runDispatchLoop` that cancels on the outer ctx OR `c.closed`, and pass `handlerCtx` into every handler closure in place of the outer `ctx`. `c.closed` fires at step 4 of the SPEC-005 §5.3 teardown unconditionally (see `protocol/disconnect.go:47`), so handler goroutines observe teardown through `SubmitWithContext`'s `ctx.Done()` arm and exit promptly. The inflightSem slot releases via the existing `defer func() { <-c.inflightSem }()`, and the `*Conn` capture becomes collectible. Read ctx (`readCtx`) is untouched — it continues to derive from the outer ctx + `c.readCtx` for `c.ws.Read` only.
- New pins: `protocol/dispatch_handler_ctx_test.go::{TestDispatchLoop_HandlerCtxCancelsOnConnClose, TestDispatchLoop_HandlerCtxCancelsOnOuterCtx}`. Primary leak pin installs a handler that blocks on its ctx.Done(), drives a subscribe frame, captures the handler ctx, asserts it does NOT return during a 25 ms window while both arms are open (proves it's parked), closes `conn.closed` directly, and asserts the handler returns within 1 s with `ctx.Err() != nil`. Secondary pin asserts outer ctx cancellation still propagates into handlerCtx. Both run green under `-race -count=3`.
- `TECH-DEBT.md` OI-004 updated: sub-hazard closed with pin anchors; dispatch-handler ctx audit removed from the remaining list; remaining OI-004 sub-hazards now narrowed to other detached-goroutine sites in `conn.go` / `lifecycle.go` / `outbound.go` / `keepalive.go` and the `ClientSender.Send` no-ctx follow-on.

Baseline note (2026-04-21): clean-tree full-suite was `Go test: 1148 passed in 10 packages` before this slice; after this slice's two new pins the baseline is `Go test: 1150 passed in 10 packages` (protocol package gained two tests).
...
With `P0-RECOVERY-001`, `P0-SCHED-001`, `P0-SUBSCRIPTION-001` closed, seven OI-005 sub-hazards closed (iter GC retention, iter use-after-Close, iter mid-iter-close, subscription-seam read-view lifetime, `CommittedSnapshot.IndexSeek` BTree-alias, `StateView.SeekIndex` BTree-alias, `StateView.SeekIndexRange` BTree-alias), the OI-006 slice-header aliasing sub-hazard closed, and six OI-004 sub-hazards closed (`watchReducerResponse` goroutine-leak, `connManagerSender.enqueueOnConn` overflow-disconnect background-ctx, `superviseLifecycle` disconnect-ctx, `ConnManager.CloseAll` disconnect-ctx — the `Background`-rooted `Conn.Disconnect` call-site family now fully closed — `forwardReducerResponse` ctx / Done lifecycle, which closes the executor-adapter twin of the earlier-closed protocol-side `watchReducerResponse` leak, and dispatch-handler ctx, which closes the request-side goroutine leak on per-message handler goroutines parked inside `executor.SubmitWithContext` under a wedged executor), the grounded options are:
...
Pick one narrow sub-hazard and land a narrow fix with a focused test, following the shape of `docs/hardening-oi-004-dispatch-handler-context.md` (latest; derived-ctx lifecycle wire), `docs/hardening-oi-004-forward-reducer-response-context.md` (Done-channel lifecycle signal precedent), or `docs/hardening-oi-004-closeall-disconnect-context.md` (bounded-ctx precedent). Concrete candidates:
...
- OI-004 remaining detached-goroutine audit in `protocol/conn.go` / `lifecycle.go` / `outbound.go` / `keepalive.go`: each would be its own narrow sub-slice if a specific leak site surfaces. The dispatch-loop and disconnect paths are now audited and pinned; the remaining lifecycle surface has not surfaced a specific seam but has not been exhaustively walked either.
...
9. `TECH-DEBT.md`
10. `docs/hardening-oi-004-dispatch-handler-context.md` (latest closed slice — derived-ctx lifecycle wire on the dispatch-loop request-side handler goroutines)
11. `docs/hardening-oi-004-forward-reducer-response-context.md` (earlier same-day OI-004 sub-slice — Done-channel lifecycle signal pattern on the executor-adapter side of the CallReducer round-trip)
12. `docs/hardening-oi-004-closeall-disconnect-context.md` (prior OI-004 sub-slice — bounded-ctx precedent on the graceful-shutdown surface; closes the `Background`-rooted `Conn.Disconnect` call-site family)
13. `docs/hardening-oi-004-supervise-disconnect-context.md` (prior OI-004 sub-slice)
14. `docs/hardening-oi-004-sender-disconnect-context.md` (earlier OI-004 sub-slice)
15. `docs/hardening-oi-004-watch-reducer-response-lifecycle.md` (earlier OI-004 sub-slice — direct precedent for the forward-reducer-response analog)
16. `docs/hardening-oi-005-state-view-seekindexrange-aliasing.md` (prior OI-005 sub-slice — slice-aliasing precedent)
17. `docs/hardening-oi-005-state-view-seekindex-aliasing.md` (prior OI-005 sub-slice — precedent)
18. `docs/hardening-oi-005-committed-snapshot-indexseek-aliasing.md` (prior OI-005 sub-slice — precedent)
19. `docs/hardening-oi-005-subscription-seam-read-view-lifetime.md` (prior OI-005 sub-slice — contract-pin precedent)
20. `docs/hardening-oi-005-snapshot-iter-mid-iter-close.md` (prior OI-005 sub-slice — precedent)
21. `docs/hardening-oi-006-fanout-aliasing.md` (prior closed Tier-B slice — precedent)
22. `docs/hardening-oi-005-snapshot-iter-useafterclose.md` (prior OI-005 sub-slice — precedent)
23. `docs/hardening-oi-005-snapshot-iter-retention.md` (earlier OI-005 sub-slice — additional precedent)
24. `docs/parity-p0-recovery-001-replay-horizon.md` (prior-closed parity slice — precedent for a narrow-and-pin parity decision doc)
25. `docs/parity-p0-sched-001-startup-firing.md` (prior-closed parity slice — alternative precedent)
26. `docs/parity-phase2-slice3-lag-policy.md` (earlier-closed parity slice — another precedent)
27. the specific code surfaces for whichever anchor (α/β/γ/δ) you pick
...
- **OI-004 dispatch-handler ctx sub-hazard (2026-04-21)** — request-side analog to `forwardReducerResponse`; `runDispatchLoop` now derives a `handlerCtx` that cancels on `c.closed`, so handler goroutines parked inside `executor.SubmitWithContext` under a wedged executor exit promptly at conn teardown
...
- **OI-004 dispatch-handler ctx sub-hazard closed** — request-side analog; `runDispatchLoop` now derives a `handlerCtx` that cancels on `c.closed`, so handler goroutines parked inside `executor.SubmitWithContext` under a wedged executor exit promptly at conn teardown. Other detached-goroutine surfaces in `conn.go` / `lifecycle.go` / `outbound.go` / `keepalive.go` and the `ClientSender.Send` no-ctx follow-on remain open under OI-004
...
- 10 packages, clean-tree full-suite baseline `Go test: 1150 passed in 10 packages` (1148 pre-slice + 2 new pins)
