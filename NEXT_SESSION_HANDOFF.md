...
No fixed implementation slice is queued after the exact-identifier and join-WHERE policy cleanups. Choose the next OI-002 batch from `TECH-DEBT.md::OI-002` by a fresh scout unless the user supplies a narrower focus. Remaining likely work is evidence-driven: projection regressions only with a fresh failing scenario, validation ordering only when it changes acceptance/rows/errors, legacy structured-query remnants, or shared fixture/test deduplication.
