...
func TestGeneratorAcceptsCanonicalContractJSON(t *testing.T) {
	...
	assertContains(t, ts, `export type TableSubscriber<Row = never> = <Name extends TableName>(table: Name, onRows?: (rows: ([Row] extends [never] ? TableRow<Name> : Row)[]) => void) => Promise<() => void>;`)
	...
	assertContains(t, ts, `export type ReducerCaller = (name: ReducerName, args: Uint8Array) => Promise<Uint8Array>;`)
	...
	assertContains(t, ts, `export type DeclaredQueryRunner = (name: ExecutableQueryName) => Promise<Uint8Array>;`)
	assertContains(t, ts, `export type DeclaredViewSubscriber = (name: ExecutableViewName) => Promise<() => void>;`)
