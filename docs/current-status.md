...
- protocol connection lifecycle races and unsafe channel-close behavior (`watchReducerResponse` goroutine-leak sub-hazard closed 2026-04-20, see `docs/hardening-oi-004-watch-reducer-response-lifecycle.md`; `connManagerSender.enqueueOnConn` overflow-disconnect background-ctx sub-hazard closed 2026-04-21, see `docs/hardening-oi-004-sender-disconnect-context.md`; broader `conn.go` / `lifecycle.go` / `outbound.go` / `keepalive.go` lifecycle concerns remain open)
