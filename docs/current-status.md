...
- Result: `Go test: 1420 passed in 10 packages`
...
- 256-bit integer column kinds `KindInt256` / `KindUint256` landed 2026-04-21 (second column-kind widening slice). `types.Value` grew a `w256 [4]uint64` slot (index 0 most-significant, signed for Int256), BSATN primitive-type tags 15 / 16 (32 bytes LE, lowest word first), coerce promotes `LitInt` via `NewInt256FromInt64` / `NewUint256FromUint64`, subscription canonical hashing writes 32 bytes big-endian. Two further `check.rs:360-370` rows accept end-to-end (`i256 = 127`, `u256 = 127`). Slice 4 (2026-04-21) closed the BigDecimal literal path: a new `LitBigInt` literal kind carries an arbitrary-precision `*big.Int`, `query/sql/parser.go::parseNumericLiteral` uses `big.Rat` to recognize integer-valued scientific shapes that overflow int64 (e.g. `1e40` → exact `10^40`), and the coerce boundary decomposes the big.Int into four uint64 words for Int256/Uint256 (with 128-bit helpers widened in parallel) and routes it to float columns via `big.Float.Float64()` so `f32 = 1e40 → +Inf` is preserved. Reference row `check.rs:330-332` (`u256 = 1e40`) now accepts end-to-end and is pinned at parser (`TestParseWhereScientificNotationOverflowBigInt`, `TestParseWhereIntegerOverflowPromotesToBigInt`), coerce (`TestCoerceBigIntLiteralToUint256`, `TestCoerceBigIntLiteralToInt256`, `TestCoerceBigIntLiteralOverflowsUint128`, `TestCoerceBigIntLiteralOverflowsUint256`, `TestCoerceNegativeBigIntRejectedOnUint256`, `TestCoerceBigIntLiteralOnInt64Rejected`, `TestCoerceBigIntLiteralToFloat32Infinity`, `TestCoerceBigIntLiteralToFloat64`, `TestCoerceBigIntLiteralOnStringColumnRejected`), and protocol admission (`TestHandleSubscribeSingle_ParityValidLiteralU256Scientific`, `TestHandleOneOffQuery_ParityValidLiteralU256Scientific`).
- Timestamp column kind `KindTimestamp` landed 2026-04-21 (third column-kind widening slice). Storage reuses the `i64` slot as microseconds since the Unix epoch; BSATN primitive-type tag 17 (8 bytes LE); coerce accepts `LitString` targeting a `KindTimestamp` column and parses RFC3339 shapes with `T` or space separator, optional fractional seconds up to nanoseconds (truncated to microseconds, matching reference `chrono::timestamp_micros`), and `Z` or numeric offset; subscription canonical hashing writes 8 bytes big-endian with the `KindTimestamp` tag separating identical raw micros from an `Int64` payload. Five reference `check.rs:334-352` rows now accept end-to-end (`ts = '2025-02-10T15:45:30Z'`, `ts = '2025-02-10T15:45:30.123Z'`, `ts = '2025-02-10T15:45:30.123456789Z'`, `ts = '2025-02-10 15:45:30+02:00'`, `ts = '2025-02-10 15:45:30.123+02:00'`).
- Array-of-string column kind `KindArrayString` landed 2026-04-21 (fifth column-kind widening slice, narrow: string element type only). Storage adds a `strArr []string` slot with defensive-copy constructor/accessor; BSATN primitive-type tag 18 (u32 LE count + repeated u32 LE length + utf8 bytes per element); coerce rejects every scalar literal shape against a `KindArrayString` column (no array literal grammar exists, the Shunter SQL surface cannot produce an ArrayString value at admission) and the `:sender` caller-identity path at the top of `coerceValue` already rejects non-KindBytes columns; subscription canonical hashing writes kind tag + u32 BE count + per-element (u32 BE length + utf8 bytes); join compile in `protocol/handle_subscribe.go::compileSQLQueryString` rejects when either join-ON column is an array kind via a new `isArrayKind` helper. Unlocks the reference parity rows `check.rs:487-489` (`select * from t where arr = :sender` / "The :sender param is an identity") and `check.rs:523-525` (`select t.* from t join s on t.arr = s.arr` / "Product values are not comparable") as positive parity contracts at the protocol admission boundary. Pinned at types round-trip / equal / compare / defensive-copy / wrong-kind panic (`TestRoundTripArrayString`, `TestArrayStringDefensiveCopyOn{Construct,Access}`, `TestEqualArrayString`, `TestCompareArrayString`, `TestAccessorArrayStringPanicsOnWrongKind`), BSATN round-trip + size + byte-order + invalid-utf8 rejection (5 new entries in `TestValueRoundTrip`, `TestEncodedValueSizeArrayString`, `TestEncodeArrayStringLittleEndianLayout`, `TestDecodeArrayStringRejectsInvalidUTF8`), coerce reject (`TestCoerceSenderRejectsArrayStringColumn`, `TestCoerceLiteralsRejectedOnArrayStringColumn`), subscription hash (`TestQueryHashArrayStringVsString`, `TestQueryHashArrayStringDiffersByPayload`, plus 3 new entries in `TestQueryHashAllKindsRoundTrip`), and protocol admission (`TestHandle{SubscribeSingle,OneOffQuery}_ParityArraySenderRejected`, `TestHandle{SubscribeSingle,OneOffQuery}_ParityArrayJoinOnRejected`). Product column kinds and array kinds with non-String element types remain unrealizable pending further widening.
