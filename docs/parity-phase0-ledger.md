...
- the next bounded A2 scout is self-join alias-sensitive join-filter associative grouping, because live `subscription/hash.go` still keeps join-containing predicates out of the broader flatten/dedupe/absorb path and the new self-join helper only normalizes immediate child order
