...
existing startup-replay / firing pins kept as parity-close; new pins
...
lock the two intentional divergences (iteration-order past-due firing
and panic-retains-row) with explicit reference citations. Remaining
sub-scenarios (`fn_start`-clamped schedule "now", one-shot panic
deletion, intended-time past-due ordering, `Workload::Internal`
commitlog labelling, lifecycle-hook startup ordering, drain-rx step)
are deferred with reference anchors in the decision doc.
