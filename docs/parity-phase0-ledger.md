...
|---|---|---|---|---|
| `P0-PROTOCOL-001` subprotocol negotiation + upgrade admission | `in_progress` | `protocol/upgrade_test.go` | WebSocket upgrade succeeds only on the supported protocol and auth path; strict mode rejects missing/invalid auth; anonymous mode mints identity/token on upgrade. | Phase 1 still must decide whether to accept SpacetimeDB protocol identifiers directly or keep the Shunter token while tightening semantics. |
| `P0-PROTOCOL-002` compression envelope + tag behavior | `in_progress` | `protocol/compression_test.go`, `protocol/sender_test.go` | Frame wrapping, compression-tag decoding, gzip round-trips, malformed/unknown compression handling, and sender framing stay explicit at the protocol boundary. | Phase 1 must still close the remaining audit-noted divergence between Shunter's envelope/tag contract and the reference protocol. |
| `P0-PROTOCOL-003` handshake / lifecycle / close behavior | `in_progress` | `protocol/lifecycle_test.go`, `protocol/close_test.go`, `protocol/reconnect_test.go`, `protocol/backpressure_in_test.go`, `protocol/backpressure_out_test.go` | Connection admission, rejection, disconnect cleanup, close-on-policy/protocol error, reconnect allowance, and backpressure-triggered shutdown are all explicit at the client-visible boundary. | Phase 1 should tighten close-code alignment and handshake semantics against the reference outcome. |
