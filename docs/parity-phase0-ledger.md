...
Phases 1 and 1.5 (envelope split) are closed. The next narrow Phase 1.5 sub-slice is the `CallReducer.flags` byte (notably `NoSuccessfulUpdate`) so clients can opt out of caller-echo delivery:
- flip `TestPhase1DeferralCallReducerNoFlagsField` once the flags field lands on the wire
- wire suppression into `subscription/fanout_worker.go` caller dispatch
- add a parity test covering the suppression path

Adjacent Phase 1.5 follow-ups (each independently landable):
