...
- The public hosted-runtime gauntlet is saturated for the current runtime,
  protocol, subscription, reducer, scheduler, recovery, read-authorization,
  observability, and clean-restart surfaces.
- Add new gauntlet work only for a new public invariant, a new feature surface,
  or a reproducible failing seed.
- Prefer focused package tests for narrow parser, codec, policy, metrics, and
  tooling behavior; reserve root gauntlets for behavior that must cross runtime,
  protocol, storage, and recovery boundaries.
- Keep fixed seeds, compact operation labels, and enough model state in
  failures to reproduce them.
- Release-candidate validation still includes full tests, pinned Staticcheck,
  targeted race/stress where relevant, fuzz corpus replay, crash/recovery
  coverage, storage-fault coverage, and at least one public hosted-runtime
  workload.
