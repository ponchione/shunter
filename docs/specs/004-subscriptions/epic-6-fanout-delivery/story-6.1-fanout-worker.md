...
- `FanOutSender`, `ReducerCallResult`, and outbound buffering are protocol-owned contracts from SPEC-005. In the live implementation, `protocol.FanOutSenderAdapter` wraps the protocol `ClientSender` and exposes the three fan-out-facing methods SPEC-004 needs: `SendTransactionUpdate`, `SendReducerResult`, and `SendSubscriptionError`. The subscription-error seam takes the full `SubscriptionError` payload, not just `(subID, message)`, so request correlation can survive reevaluation failures.
