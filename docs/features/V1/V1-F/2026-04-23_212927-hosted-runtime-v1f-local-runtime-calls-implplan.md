...
- `docs/specs/hosted-runtime-version-phases.md` defines V1-F as local runtime calls after V1-C/V1-D runtime ownership and before V1-G export/introspection.
...
- `executor.Executor.SubmitWithContext(ctx, cmd)` is the external admission entrypoint used by protocol paths and is rejected with `ErrExecutorNotStarted` until `Startup` completes. The landed root local-call implementation uses this path so local calls preserve startup admission and permission behavior.
...
2. Local reducer calls use the executor command path.
   ...
   - The landed implementation submits through `Executor.SubmitWithContext` rather than `Submit` so context cancellation, startup admission, and permission behavior match the external reducer path.
...
Files:
- Read: this plan
- Read: V1-D and V1-E plans under `docs/features/V1/`
...
func (r *Runtime) CallReducer(ctx context.Context, name string, args []byte, opts ...CallReducerOption) (ReducerResult, error) {
    ...
    if err := r.executor.SubmitWithContext(ctx, cmd); err != nil {
        return ReducerResult{}, err
    }
...
Files:
- Modify: `runtime_local.go`
- Possibly modify: `docs/specs/hosted-runtime-v1-contract.md` only if implementation settles names that differ from the contract
...
V1-F plan as a live handoff; use the relevant feature plan for
