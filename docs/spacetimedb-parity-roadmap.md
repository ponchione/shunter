...
- one-off SQL now also accepts the bounded composition of the already-supported `COUNT(*) [AS] alias` aggregate projection (single-table, join-backed, and cross-join `WHERE` shapes) with a trailing unsigned-integer `LIMIT`, for example `SELECT COUNT(*) AS n FROM t WHERE active = TRUE LIMIT 1` or `SELECT COUNT(*) AS n FROM t JOIN s ON t.id = s.t_id LIMIT 1`: the parser already carries both `Statement.Aggregate` and `Statement.Limit`, one-off counts matched rows with multiplicity into a one-column uint64 aggregate result and then applies `LIMIT` to the one-row aggregate output (reference `ProjectList::Limit` over `ProjectList::Agg(Count)`, so `LIMIT 0` drops the aggregate row and `LIMIT >= 1` keeps the single count row), and subscribe still rejects via the `LIMIT`-not-supported guard (which fires before the aggregate-projection guard) before executor registration
