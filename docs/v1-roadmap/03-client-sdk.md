...
does not yet implement typed reducer argument/result encoding, typed declared
query row decoding, declared-view/table-subscription protocol plumbing, auth
refresh, local cache updates, or reconnect policy.
...
- Implement typed reducer result decoding beyond the current raw
  `TransactionUpdate` frame result, plus declared-query row decoding and
