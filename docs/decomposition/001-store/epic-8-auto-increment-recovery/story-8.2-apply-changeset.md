  ...
  **Algorithm:**
  ...
  2. For each `TableChangeset` in `cs.Tables`:
     ...
     c. For each insert row:
        ...
        - If the row contains a value in an auto-increment column (SPEC-006 §9 / Story 8.1), advance `Sequence.next := max(Sequence.next, observed+1)` so post-recovery auto-assign does not reissue a value already persisted in a replayed insert (SPEC-AUDIT §2.1).
...
- [ ] Replay inserts with sequence-column values N, N-3, N-7 (out of order) → post-replay next auto-assign yields N+1
- [ ] Snapshot-restored `Sequence.next = N-5`, then replay insert with sequence-column value N → post-replay next auto-assign yields N+1, not N-4
...
- The alternative sequence model (SpacetimeDB's `allocated` upper bound stored alongside `next`) is deferred to v2. The v1 approach leans on replay to advance the counter, which requires the exactly-once replay contract below.
